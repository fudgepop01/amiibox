...
const index = require('./index-41b9663e.js');
...
const HexEditor = class {
    constructor(hostRef) {
        ...
        this.hexLineChanged = index.createEvent(this, "hexLineChanged", 7);
        this.hexCursorChanged = index.createEvent(this, "hexCursorChanged", 7);
        this.hexSelectionChanged = index.createEvent(this, "hexSelectionChanged", 7);
        this.hexDataChanged = index.createEvent(this, "hexDataChanged", 7);
        this.hexLoaded = index.createEvent(this, "hexLoaded", 7);
    }
    // !SECTION
    ...
    */
    ...
    buildHexView() {
            ...
            for (const [position, val] of [...line.values()].entries()) {
                ...
                if (this.displayBin) {
                    for (let i = 0; i < binArr.length; i++) {
                        ...
                        binSpans.push(index.h("span", { "data-cursor-idx": i, class: binClass }, binArr[i]));
                    }
                }
                if (this.displayBin)
                    binLines.push(index.h("span", { "data-cursor-idx": base + position, class: "binGroup" + (classList.includes('added') ? ' added' : '') }, binSpans));
                if (this.displayAscii)
                    charLines.push(index.h("span", { "data-cursor-idx": base + position, class: classList.join(' ') }, ascii));
                if (this.displayHex)
                    hexLines.push(index.h("span", { "data-cursor-idx": base + position, class: classList.join(' ') }, out));
            }
            if (this.displayBin)
                binViews.push((index.h("div", { class: 'binLine' + (selectedLine === lineNum ? ' selected' : '') }, binLines)));
            if (this.displayHex) {
                lineViews.push((index.h("div", { class: 'hexLine' + (selectedLine === lineNum ? ' selected' : '') }, hexLines)));
            }
            else {
                lineViews.push({});
            }
            if (this.displayAscii)
                charViews.push((index.h("div", { class: 'charLine' + (selectedLine === lineNum ? ' selected' : '') }, charLines)));
        }
        // fill extra space
        while (lineViews.length < maxLines) {
            binViews.push(index.h("div", { class: "binLine", style: { pointerEvents: 'none' } }, index.h("span", null, "-")));
            lineViews.push(index.h("div", { class: "hexLine", style: { pointerEvents: 'none' } }, index.h("span", null, "-")));
            charViews.push(index.h("div", { class: "charLine", style: { pointerEvents: 'none' } }, index.h("span", null, "-")));
        }
        // line number builder
        const lineLabels = [];
        for (let i = 0; i < maxLines; i++) {
            lineLabels.push(index.h("div", { class: 'lineLabel' + (selectedLine === i ? ' selected' : ''), style: { pointerEvents: 'none' } }, '0x' + (start + i * bytesPerLine).toString(16).padStart(8, ' ')));
        }
        // regions
        const binRegionMarkers = [];
        const hexRegionMarkers = [];
        const asciiRegionMarkers = [];
        const buildRegion = (region, depth = 0, index$1) => {
            ...
            const genPolygon = (width, height) => (index.h("polygon", { onMouseMove: (evt) => {
              ...
              0,${(l + 1 + offset) * height}
            `, fill: region.color || getColor[depth % 3][index$1 % 2], stroke: "none" }));
        ...
        const binRegions = index.h("svg", { viewBox: `0 0 ${this.bytesPerLine * 14 * 8} ${this.maxLines * this.regionScaleHeight}`, width: `${this.bytesPerLine * 14 * 8}`, height: `${this.maxLines * this.regionScaleHeight}` }, binRegionMarkers);
        const hexRegions = index.h("svg", { viewBox: `0 0 ${this.bytesPerLine * this.regionScaleWidth} ${this.maxLines * this.regionScaleHeight}`, width: `${this.bytesPerLine * this.regionScaleWidth}`, height: `${this.maxLines * this.regionScaleHeight}` }, hexRegionMarkers);
        const asciiRegions = index.h("svg", { viewBox: `0 0 ${this.bytesPerLine * 10} ${this.maxLines * this.regionScaleHeight}`, width: `${this.bytesPerLine * 10}`, height: `${this.maxLines * this.regionScaleHeight}` }, asciiRegionMarkers);
        return {
            lineViews,
            charViews,
            binViews,
    ...
    buildChunks() {
        ...
        for (const { start, data, startLine } of renderedChunks) {
            if (start === -1) {
                lineLabels.push(index.h("div", { class: 'separator', style: { pointerEvents: 'none' } }, "NA"));
                lineViews.push(index.h("div", { class: 'separator', style: { pointerEvents: 'none' } }, "NA"));
                charViews.push(index.h("div", { class: 'separator', style: { pointerEvents: 'none' } }, "NA"));
                binViews.push(index.h("div", { class: 'separator', style: { pointerEvents: 'none' } }, "NA"));
                continue;
            }
                ...
                for (let j = i; j < i + bytesPerLine && j < data.length; j++) {
                    ...
                    if (this.displayBin) {
                        for (let k = 0; k < binArr.length; k++) {
                            ...
                            binSpans.push(index.h("span", { "data-cursor-idx": k, class: binClass }, binArr[k]));
                        }
                    }
                    if (this.displayBin)
                        binLine.push(index.h("span", { "data-cursor-idx": position, class: "binGroup" + (classList.includes('added') ? ' added' : '') }, binSpans));
                    if (this.displayAscii)
                        charLine.push(index.h("span", { "data-cursor-idx": position, class: classList.join(' ') }, ascii));
                    if (this.displayHex)
                        hexLine.push(index.h("span", { "data-cursor-idx": position, class: classList.join(' ') }, out));
                }
                lineLabels.push((index.h("div", { class: 'lineLabel' + (selectedLine === lineStart ? ' selected' : ''), style: { pointerEvents: 'none' } }, '0x' + (lineStart).toString(16).padStart(8, ' '))));
                if (this.displayBin)
                    binViews.push((index.h("div", { class: 'binLine' + (selectedLine === lineStart ? ' selected' : '') }, binLine)));
                if (this.displayHex) {
                    lineViews.push((index.h("div", { class: 'hexLine' + (selectedLine === lineStart ? ' selected' : '') }, hexLine)));
                }
                else {
                    lineViews.push({});
                }
                if (this.displayAscii)
                    charViews.push((index.h("div", { class: 'charLine' + (selectedLine === lineStart ? ' selected' : '') }, charLine)));
            }
            const buildRegion = (region, depth = 0, index$1) => {
        ...
        while (lineViews.length < maxLines) {
            lineLabels.push(index.h("div", { class: "separator" }, index.h("span", null, "-")));
            binViews.push(index.h("div", { class: "separator" }, index.h("span", null, "-")));
            lineViews.push(index.h("div", { class: "separator" }, index.h("span", null, "-")));
            charViews.push(index.h("div", { class: "separator" }, index.h("span", null, "-")));
        }
        const binRegions = index.h("svg", { viewBox: `0 0 ${this.bytesPerLine * 14 * 8} ${this.maxLines * this.regionScaleHeight}`, width: `${this.bytesPerLine * 14 * 8}`, height: `${this.maxLines * this.regionScaleHeight}` }, binRegionMarkers);
        const hexRegions = index.h("svg", { viewBox: `0 0 ${this.bytesPerLine * this.regionScaleWidth} ${this.maxLines * this.regionScaleHeight}`, width: `${this.bytesPerLine * this.regionScaleWidth}`, height: `${this.maxLines * this.regionScaleHeight}` }, hexRegionMarkers);
        const asciiRegions = index.h("svg", { viewBox: `0 0 ${this.bytesPerLine * 10} ${this.maxLines * this.regionScaleHeight}`, width: `${this.bytesPerLine * 10}`, height: `${this.maxLines * this.regionScaleHeight}` }, asciiRegionMarkers);
    ...
    showHex() {
        ...
        if (this.searchActive) {
            ...
            searchResults = (index.h("select", { onChange: (evt) => jumpToResult(evt.target.value) }, this.searchResults.map(v => index.h("option", { value: v }, `0x${v.toString(16)}`))));
        }
        return (index.h("div", { class: "hex", onMouseEnter: (evt) => this._toggleScrollListener(evt), onMouseLeave: (evt) => this._toggleScrollListener(evt), onMouseDown: (evt) => this.beginSelection(evt), onMouseUp: (evt) => this.endSelection(evt), tabindex: "0", onKeyDown: (evt) => this.edit(evt) }, index.h("div", { id: "MEASURE", class: "hex", style: { position: 'absolute', visibility: 'hidden', padding: '0 5px' } }, "AB"), index.h("div", { class: "lineLabels" }, lineLabels), this.displayBin ?
            index.h("div", { class: "binView" }, index.h("div", { class: "highlight", style: { position: 'absolute', top: '0', display: this.mode === 'noregion' ? 'none' : 'block', zIndex: this.mode === 'region' ? '3' : '0' } }, binRegions), index.h("div", { class: "main" }, binViews))
            : null, this.displayHex ?
            index.h("div", { class: "hexView" }, index.h("div", { class: "highlight", style: { position: 'absolute', top: '0', display: this.mode === 'noregion' ? 'none' : 'block', zIndex: this.mode === 'region' ? '3' : '0' } }, hexRegions), index.h("div", { class: "main" }, lineViews))
            : null, this.displayAscii ?
            index.h("div", { class: "asciiView" }, index.h("div", { class: "highlight", style: { position: 'absolute', top: '0', display: this.mode === 'noregion' ? 'none' : 'block', zIndex: this.mode === 'region' ? '3' : '0' } }, asciiRegions), index.h("div", { class: "main" }, charViews))
            : null, this.searchActive ?
            index.h("div", { class: "find" }, "search:", index.h("input", { type: "text", onChange: (evt) => this.searchInput = evt.target.value }), index.h("select", { onChange: (evt) => this.searchType = evt.target.value }, index.h("option", { value: "ascii" }, "ASCII string"), index.h("option", { value: "byte" }, "bytes"), index.h("option", { value: "integer" }, "integer"), index.h("option", { value: "float" }, "float")), (['integer', 'float'].includes(this.searchType)) ? [
                index.h("select", { onChange: (evt) => this.searchByteCount = parseInt(evt.target.value) }, index.h("option", { value: "1" }, "1 byte"), index.h("option", { value: "2" }, "2 bytes"), index.h("option", { value: "4" }, "4 bytes"), index.h("option", { value: "8" }, "8 bytes")),
                index.h("select", { onChange: (evt) => this.searchEndian = evt.target.value }, index.h("option", { value: "big" }, "big endian"), index.h("option", { value: "little" }, "little endian"))
            ]
                : null, index.h("button", { onClick: () => this.findInSelection() }, "search"), index.h("br", null), "hex: ", searchHexDisplay, " | results: ", searchResults)
            : null));
    }
    /**
     * displays the chunks
    ...
    render() {
        ...
        if (this.displayAsChunks)
            ...
            out = this.showHex();
        return (index.h("div", { class: "fudgedit-container" }, out));
    }
...
const Tooltip = class {
    ...
    render() {
        ...
        if (this.data) {
            ...
            if (data.name)
                out.push(index.h("span", null, `name: ${data.name}`), index.h("br", null));
            out.push(index.h("span", null, `size: ${data.end - data.start} [0x${data.start.toString(16)} - 0x${data.end.toString(16)}]`), index.h("br", null));
            for (const [key, value] of Object.entries(data)) {
                if (['name', 'subRegions', 'start', 'end'].includes(key))
                    continue;
                if (value !== null) {
                    out.push(index.h("span", null, key, ": ", value), index.h("br", null));
                }
            }
        }
        else if (this.simpleText) {
            out.push(index.h("span", null, this.simpleText));
        }
        else {
            out.push(index.h("span", null, "placeholder"));
        }
        return out;
    }
};
...
exports.fudge_hex_editor = HexEditor;
exports.fudge_hex_tooltip = Tooltip;
